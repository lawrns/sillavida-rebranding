    ...
    <div className="product-features">
      <div className="flex flex-col gap-8">
        {features.map(feature => (
          <div key={feature.id} className="flex flex-col md:flex-row gap-6 items-center bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
            <div className="w-full md:w-1/3 flex justify-center">
              <img 
                src={feature.image} 
                alt={feature.title} 
                className="w-full max-w-[200px] h-auto object-contain rounded-lg bg-gray-50 p-4" 
                onError={(e) => {
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-heading font-semibold text-xl text-teal">{feature.title}</h3>
              </div>
              <p className="font-body text-gray-700 mb-4 leading-relaxed">{feature.description}</p>
              {feature.vidaScore && (
                  ...
                  {Object.entries(feature.vidaScore)
                    ...
                    .map(([cat, score]) => (
                      <div key={cat} className="flex items-center">
                        <span className="bg-sage text-white px-3 py-1 rounded-full font-bold">{cat}</span>
