...
const ShopifyPromoBanner: React.FC<ShopifyPromoBannerProps> = ({ product, dark = false }) => {
  ...
  return (
    <div className={`relative overflow-hidden vida-shape-organic ${dark ? 'bg-[#F5F5F5] vida-bg-pattern-breathing' : 'bg-[#F5F5F5] vida-bg-pattern-leaf'}`}>
      <div className="flex flex-col md:flex-row items-center justify-between p-8">
        <div className="flex-1 mb-8 md:mb-0">
          <div className="inline-block px-4 py-1 rounded-full bg-[#666666] text-[#FFFFFF] text-sm mb-4 font-heading font-medium">
            Campeón de Ventas
          </div>
          <h3 className="text-3xl font-heading font-bold mb-4 text-[#000000]">
            {product.title}
          </h3>
          <ul className="vida-feature-list space-y-2 mb-6 text-[#333333] font-body">
          ...
          <Link to={`/product/${product.handle}`}>
            <button
              className="mt-6 px-8 py-3 bg-[#000000] text-[#FFFFFF] vida-shape-soft hover:opacity-90 transition-colors font-heading font-semibold tracking-wide"
        ...
        <div className="flex-1 relative">
          {!imageLoaded && !imageError && (
            <div className="w-full h-[400px] bg-gray-200 animate-pulse flex items-center justify-center">
              <span className="text-[#666666] font-body">Cargando imagen...</span>
            </div>
          )}
          {imageError && (
            <div className="w-full h-[400px] bg-gray-200 flex items-center justify-center">
              <span className="text-[#666666] font-body">Imagen no disponible</span>
            </div>
          )}
          <img
            src={imageUrl}
