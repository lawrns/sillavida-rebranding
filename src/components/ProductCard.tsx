      ...
      <Link to={`/product/${chair.id}`} className="block"> {/* Link remains block inside motion div */}
        ...
        <div className="p-4">
          <div className="flex items-center mb-2">
            ...
            <span className="ml-2 text-sm text-gray-600 font-body">{chair.rating}</span>
          </div>
          {/* Life Category Badge */}
          <div className="mb-2">
            <span className="inline-block px-2 py-1 text-xs font-medium bg-teal-light/20 text-teal-dark rounded-full font-body">
              {chair.lifeCategory}
            </span>
          </div>
          
          <h3 className="font-heading font-semibold mb-1 product-title">{chair.name}</h3>
          
          {/* Primary Benefit Statement */}
          <p className="text-teal font-medium mb-2 text-sm font-heading">
            {chair.primaryBenefit}
          </p>
          
          <p className="text-gray-600 mb-3 line-clamp-2 flex-grow font-body product-description">
          ...
          {chair.benefitCategories && (
            <div className="mb-3">
              <ul className="vida-feature-list text-sm text-gray-700">
                {chair.benefitCategories.health && chair.benefitCategories.health.length > 0 && (
                  <li className="font-body text-xs flex items-start mb-1">
                    <span className="inline-block w-2 h-2 rounded-full bg-sage mt-1 mr-2"></span>
                ...
                {chair.benefitCategories.productivity && chair.benefitCategories.productivity.length > 0 && (
                  <li className="font-body text-xs flex items-start mb-1">
                    <span className="inline-block w-2 h-2 rounded-full bg-teal mt-1 mr-2"></span>
          ...
          <div className="mt-auto pt-2"> {/* Push button to bottom, added padding top */}
            ...
            disabled={isLoading}
            className={`w-full mt-4 py-2 vida-shape-soft transition-colors font-heading font-semibold tracking-wide ${
              success 
